...
if nb_groups==2: #2 groups selection
    ...
    aov_table['index']=aov_table.index
    first_column = aov_table.pop('index')
    aov_table.insert(0, 'index', first_column)
    anova_list = {}
    anova_list['header'] = list(aov_table.head(1))
    anova_list_data = []
    for index, row in aov_table.iterrows():
        anova_list_data.append(list(row))
    anova_list['data'] = anova_list_data
...
final_tab = final_tab[final_tab[id_variable].notna()]
final_tab['id_pct'] = 1/final_tab.shape[0]


groups_count_table_list = {}
groups_count = pd.pivot_table(final_tab,values='id_pct',index='group',aggfunc='sum')
groups_count = groups_count.round(decimals=2)
groups_count_table_list['headers'] = []
groups_count_table_list['data_series'] = []

for index, row in groups_count.iterrows():
    groups_count_table_list['headers'].append(index)
    
    groups_count_dict = {}
    groups_count_dict['name'] = index
    groups_count_dict['data'] = list(row)
    groups_count_table_list['data_series'].append(groups_count_dict)

final_output['groups_count_table_list'] = groups_count_table_list

#--- create table for chart in the dashboard---------
pivot_tables_list={}
for i in range(0,len(final_strata_vars.split(','))):   
    chart_data = {}
    table1 = (pd.pivot_table(final_tab,values='id_pct', index='group', columns=final_strata_vars.split(',')[i], aggfunc='sum')).round(decimals=3)
    chart_data['headers'] = list(table1.head(1))
    data_series = []
    for index, row in table1.iterrows():
        dict1 = {}
        dict1["name"] = index
        dict1["data"] = list(row)
        data_series.append(dict1)

    chart_data['data_series'] = data_series

    pivot_tables_list[final_strata_vars.split(',')[i]] = chart_data

final_output['pivot_tables_list'] = pivot_tables_list


#create the summary table---
summary_table = final_tab.groupby('group', as_index = False).agg({id_variable:'count'})
for i in range(0,len(final_numvars)):
    grp_sum=final_tab.groupby('group',as_index=False).agg({final_numvars[i]:'sum'})
    summary_table=pd.merge(summary_table, grp_sum, on='group', how='left')
    summary_table['avg_' + final_numvars[i]] = summary_table[final_numvars[i]]/summary_table[id_variable]

summary_table = summary_table.round(decimals = 3) 
summary_table.set_index('group', inplace=True)
summary_table_list = {}
summary_table_list['headers'] = list(summary_table.head(1))
summary_table_list_data = []

for index, row in summary_table.iterrows():
    summary_table_list_data.append(list(row))
summary_table_list['data'] = summary_table_list_data

final_output['summary_table_list'] = summary_table_list

#json=json.dumps(final_output, indent=4) 
json = simplejson.dumps(final_output, ignore_nan = True)
print(json)
